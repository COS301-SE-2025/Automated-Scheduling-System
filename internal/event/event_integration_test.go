...
func setupRouter() *gin.Engine {
	...
	api.Use(mockAuthMiddleware())
	{
		...
		// Newly added extended endpoints
		api.POST("/event-schedules/:scheduleID/rsvp", RSVPHandler)
		api.GET("/event-schedules/:scheduleID/attendance", GetAttendanceHandler)
		api.POST("/event-schedules/:scheduleID/attendance", SetAttendanceHandler)
		api.GET("/event-schedules/:scheduleID/attendance-candidates", GetAttendanceCandidates)
		api.POST("/competency-check", CheckEmployeesHaveCompetency)
	}
	return r
}
...
// --- Extended Endpoint Integration Tests ---

func TestExtendedEventEndpoints(t *testing.T) {
	db := setupTestDB(t)
	user, definition, schedule := seedData(t, db)
	r := setupRouter()

	// Stub context as Admin for unrestricted access
	original := currentUserContextFn
	currentUserContextFn = func(c *gin.Context) (*gen_models.User, *gen_models.Employee, bool, bool, error) {
		var u gen_models.User
		require.NoError(t, db.Where("username = ?", testUsername).First(&u).Error)
	...
	t.Run("RSVP book open event", func(t *testing.T) {
		path := fmt.Sprintf("/api/event-schedules/%d/rsvp", schedule.CustomEventScheduleID)
		body := []byte(`{"choice":"book"}`)
		w := performRequest(r, "POST", path, body, headers)
		require.Equal(t, http.StatusOK, w.Code)
		// Verify DB row created with role='Booked'
		var count int64
		err := db.Table("event_schedule_employees").Where("custom_event_schedule_id = ? AND employee_number = ? AND role = ?", schedule.CustomEventScheduleID, user.EmployeeNumber, "Booked").Count(&count).Error
		require.NoError(t, err)
		require.EqualValues(t, 1, count)
	})

	t.Run("Get attendance (initially empty)", func(t *testing.T) {
		path := fmt.Sprintf("/api/event-schedules/%d/attendance", schedule.CustomEventScheduleID)
		w := performRequest(r, "GET", path, nil, headers)
		require.Equal(t, http.StatusOK, w.Code)
	})

	t.Run("Set attendance for booked user", func(t *testing.T) {
		path := fmt.Sprintf("/api/event-schedules/%d/attendance", schedule.CustomEventScheduleID)
		payload := fmt.Sprintf(`{"employeeNumbers":["%s"],"attendance":{"%s":true}}`, user.EmployeeNumber, user.EmployeeNumber)
		w := performRequest(r, "POST", path, []byte(payload), headers)
		require.Equal(t, http.StatusOK, w.Code)
		// Confirm attendance row
		var attCount int64
		err := db.Table("event_attendance").Where("custom_event_schedule_id = ? AND employee_number = ? AND attended = ?", schedule.CustomEventScheduleID, user.EmployeeNumber, true).Count(&attCount).Error
		require.NoError(t, err)
		require.EqualValues(t, 1, attCount)
	})

	t.Run("Attendance candidates endpoint", func(t *testing.T) {
		path := fmt.Sprintf("/api/event-schedules/%d/attendance-candidates", schedule.CustomEventScheduleID)
		w := performRequest(r, "GET", path, nil, headers)
		require.Equal(t, http.StatusOK, w.Code)
	})

	t.Run("Competency check endpoint returns map", func(t *testing.T) {
		body := []byte(`{"competencyId":9999,"employeeNumbers":["` + user.EmployeeNumber + `"]}`)
		w := performRequest(r, "POST", "/api/competency-check", body, headers)
		require.Equal(t, http.StatusOK, w.Code)
		// Response should be JSON object; we won't assert specific keys since table may be empty.
		require.NotEmpty(t, w.Body.Bytes())
	})

	t.Run("Create additional schedule then RSVP", func(t *testing.T) {
		start := time.Now().Add(48 * time.Hour)
		req := models.CreateEventScheduleRequest{CustomEventID: definition.CustomEventID, Title: "Second", EventStartDate: start, EventEndDate: start.Add(2 * time.Hour)}
		b, _ := json.Marshal(req)
		wCreate := performRequest(r, "POST", "/api/event-schedules", b, headers)
		require.Equal(t, http.StatusCreated, wCreate.Code)
		// Extract first schedule ID from response array
		var scheds []models.CustomEventSchedule
		_ = json.Unmarshal(wCreate.Body.Bytes(), &scheds)
		require.GreaterOrEqual(t, len(scheds), 1)
		targetID := scheds[len(scheds)-1].CustomEventScheduleID
		path := fmt.Sprintf("/api/event-schedules/%d/rsvp", targetID)
		w := performRequest(r, "POST", path, []byte(`{"choice":"book"}`), headers)
		require.Equal(t, http.StatusOK, w.Code)
	})
}
